...
SpreadDistance = 0 #如果超过这个数值，我们认为差距过大
...
class AutoTrader(BaseAutoTrader):
    ...
    def Trade_by_trade_ticks_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
                                     ask_volumes: List[int], bid_prices: List[int], bid_volumes: List[int]):
        """ Author: AbsoluteX
        增加我们交易的频率
        依据tick信息做单边交易 只有在差价很大的时候交易才会被触发
        """
        new_ask_price = 0
        new_bid_price = 0
        if self.orderbook[Instrument.ETF]['seq'] is None: return new_ask_price, new_bid_price #防止交易出现问题
        if self.orderbook[Instrument.ETF]['seq'] <2 : return new_ask_price, new_bid_price #防止交易出现问题
        if instrument==Instrument.FUTURE: return new_ask_price, new_bid_price #只交易ETF数据发生改变的情况

        ### 在0处促进成交
        # 当 ask orderbook 发生更改，我们尝试更新ask订单
        if sum(ask_prices)>0:
            #print(max(ask_prices) , self.predict_orderbook[Instrument.FUTURE]['askprc'][0] , SpreadDistance*TICK_SIZE_IN_CENTS)
            if max(ask_prices) - self.predict_orderbook[Instrument.FUTURE]['askprc'][0] >= SpreadDistance*TICK_SIZE_IN_CENTS:
                new_ask_price = max(ask_prices)
                #print('begin_try_predict_orderbokk, set ask order')
                return new_ask_price, new_bid_price
        
        #当 bid orderbook 发生更改，我们尝试更新bid订单
        if sum(bid_prices)>0:
            min_bidprc = min(x for x in bid_prices if x>0)
            if min_bidprc - self.predict_orderbook[Instrument.FUTURE]['bidprc'][0] <= -SpreadDistance*TICK_SIZE_IN_CENTS:
                new_bid_price = min_bidprc
                #print('begin_try_predict_orderbokk, set bid order')
                return new_ask_price, new_bid_price
    ...
    def on_trade_ticks_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        """
        #trading here
        if self.has_trade<1:  #防止过量交易
            self.has_trade+=1
            self.get_order_num()  # Update the order number we will put, according to current position
            ask_prc2, bid_prc2 = self.Trade_by_trade_ticks_message(instrument, sequence_number, ask_prices,ask_volumes, bid_prices, bid_volumes)
            self.send_order('ETF_Future_spread_strategy',ask_prc2, bid_prc2, self.ask_num, self.bid_num)
